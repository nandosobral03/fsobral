...
export default function ProjectCard({ title, image, children, year, variant = "carousel" }: ProjectCardProps) {
  ...
  return (
    <Link href={`/projects/${title}`} className={`${widthClass} h-[420px] md:h-[480px] flex flex-col bg-foreground text-background group select-none overflow-hidden`}>
      ...
      <div className="flex flex-col gap-2 p-4 md:p-5 flex-1">
        <div className="flex items-baseline justify-between">
          ...
          <span className="meta-label text-background/40">{year}</span>
        </div>
